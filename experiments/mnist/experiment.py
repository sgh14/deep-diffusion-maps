...
encoder = build_conv_encoder(input_shape=X_a.shape[1:], filters=8, n_components=n_components, zero_padding=(2, 2), dropout=0.2, use_bn=False)
tic = time.perf_counter()
loss = DiffusionLoss(X_a, sigma=sigma, steps=steps, alpha=alpha)
encoder.compile(loss=loss, optimizer=tf.keras.optimizers.Adam(learning_rate=0.05))
