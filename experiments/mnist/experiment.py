...
from experiments.mnist.load_data import get_data
from experiments.utils.models import build_conv_encoder
...
def main() -> None:
    ...
    ddm_results, encoder, history = deep_diffusion_maps_experiment(
        ...
        steps=hyperparameters['steps'],
        alpha=hyperparameters['alpha'],
        encoder_builder=build_conv_encoder,
    ...
    nystrom_results = nystrom_experiment(
        ...
        alpha=hyperparameters['alpha'],
        reference_results=dm_results,
        reference_coords=np.zeros(data['train']['X'].shape[1:])
    )
